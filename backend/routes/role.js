...
router.get("/roles", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        res.json({ success: true, data: roles, message: "取得角色列表成功" });
    } catch (error) {
...
router.post("/roles", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        if (!user_id || !role_id)
            return res.status(400).json({ success: false, message: "缺少 user_id 或 role_id" });
        ...
        if (role.length === 0)
            return res.status(404).json({ success: false, message: "找不到角色" });
        ...
        res.json({ success: true, message: "已成功指派角色" });
    } catch (err) {
...
router.put("/roles", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        res.json({ success: true, message: "已成功更新角色" });
    } catch (err) {
...
router.delete("/roles", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        res.json({ success: true, message: "已成功刪除角色" });
    } catch (err) {
